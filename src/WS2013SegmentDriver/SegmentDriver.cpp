...
void SegmentDriver::loop()
{
    ...
    //setTime(11,50);
  /*  for(int16_t z = 0; z < NUM_LEDS; z++)
    {
        if(leds[z].r != 0 && leds[z].g != 0 && leds[z].b != 0)
        {
            leds[z] = CHSV( gHue, 255, 255);
        }
    }*/
  //Serial.println();
 // delay(1000);
  ...
  EVERY_N_MILLISECONDS( 250 ) { gHue++; } // slowly cycle the "base color" through the rainbow
}
  ...
  for(int16_t i = 0; i < 7; i++)
    {
        ...
        //Serial.println(digitDefinitions[desiredNumber][i].leds[0]);
        if(digitDefinitions[value][i].leds[0] != -1)
        {
          for(int16_t y = 0; y < 3; y++)
          {
              //Serial.print(digitDefinitions[desiredNumber][i].leds[y]);
          // Serial.print("  ");
